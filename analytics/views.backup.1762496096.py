...
class OwnerRentalPropertyPatchView(APIView):
    ...
    def patch(self, request, pk: int):
        Property = _get_property_model()
        if Property is None:
            return Response({"detail": "model not available"}, status=400)

        body = request.data or {}


        updatable = {
            "rent": "monthly_rent" if hasattr(Property, "monthly_rent") else ("expected_rent" if hasattr(Property, "expected_rent") else None),
            "igen_service_charge": "igen_service_charge" if hasattr(Property, "igen_service_charge") else None,
            "lease_start": "lease_start_date" if hasattr(Property, "lease_start_date") else ("lease_start" if hasattr(Property, "lease_start") else None),
            "lease_expiry": "lease_end_date" if hasattr(Property, "lease_end_date") else ("lease_expiry" if hasattr(Property, "lease_expiry") else None),

    # ✅ NEW: inline, auto-save toggles
            "transaction_scheduled": "transaction_scheduled" if hasattr(Property, "transaction_scheduled") else None,
            "email_sent": "email_sent" if hasattr(Property, "email_sent") else None,
       }

        # Drop Nones (fields not present in current model)
        ...
        for k, field in updatable.items():
            if k in body and hasattr(obj, field):
                setattr(obj, field, body[k])
