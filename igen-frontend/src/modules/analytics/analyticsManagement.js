...
// -------- NEW: Property-based statement helpers for Owner Dashboard --------
async function openPropertyStatementPDF(propertyId, month) {
  if (!propertyId) {
...
async function openPropertyStatementDOCX(propertyId, month) {
  if (!propertyId) {
    alert("Property not found for this row.");
    return;
  }
...
function OwnerRentalTab() {
  ...
  const generateRowStatement = async (propertyId, format) => {
    ...
    try {
      ...
      if (format === "word") {
        await openPropertyStatementDOCX(propertyId, month);
      } else {
        await openPropertyStatementPDF(propertyId, month);
      }
    } catch (e) {
      console.error(e);
      alert("Failed to generate statement.");
  ...
  return (
    <>
      ...
      <Card
        ...
        subtitle="Inline edit for Rent / iGen SC / Lease dates & renewal. Generate statement per property."
      >
        <Table
          headers={[
            "Property",
                  ...
                  <td className="px-3 py-2 flex items-center gap-2">
                    {!isEditing ? (
                      <>
                        ...
                        <ExportMenu
                          label="Generate"
                          title="Generate monthly statement for this property"
                          options={[
                            {
                              label: "PDF",
