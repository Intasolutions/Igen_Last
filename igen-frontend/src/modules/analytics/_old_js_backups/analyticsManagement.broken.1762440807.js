...
// - Opens downward (below the input), right-aligned
...
function MultiSelect({ label, options, value, onChange, placeholder = "Select..." }) {
  ...
  const display = (value || []).slice(0, 2).map((v) => options.find((o) => o.value === v)?.label || v).join(", ");
...
function FinancialDashboard() {
  ...
  // load distinct values for a dimension via existing pivot API
  ...
  const runPivot = async () => {
    ...
    try {
      ...
      setLoading(falselDims = dateOn ? [...dims, "date"] : dims;
      const res = await API.post(
        "analytics/pivot/export/",
        { dims: finalDims, values: buildValuesPayload(), from, to, date_granularity: dateOn ? gran : null },
        { responseType: "blob" }
      );
      const url = URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement("a");
