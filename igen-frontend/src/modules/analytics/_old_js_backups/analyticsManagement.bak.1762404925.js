...
// --- shared helpers so all sections can use them safely ---
const genCurrentMonth = () => new Date().toISOString().slice(0, 7);

async function generateStatement(entityId, propertyName) {
  let id = entityId;
  if (!id) {
    const input = window.prompt(
      `No entity is linked to "${propertyName ?? "this property"}". Enter the Entity ID to generate the statement:`
    );
    if (!input) return;
...
// Uses GET /api/analytics/entities/search/?q=...&limit=20 (already implemented on your backend)
function EntityPicker({ value, onChange, placeholder = "Search entity" }) {
  const [open, setOpen] = useState(false);
  const [options, setOptions] = useState([]); // [{id,name}]
  const [q, setQ] = useState("");
  const [loading, setLoading] = useState(false);

  // simple debounce & fetch
  useEffect(() => {
    if (!open) return;
    const t = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await API.get("analytics/entities/search/", { params: { q, limit: 20 } });
        setOptions(res.data || []);
      } catch (e) {
        console.error(e);
        setOptions([]);
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-[320px] max-h-64 overflow-auto rounded-lg border border-gray-200 bg-white shadow">
...
function EntityStatement() {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `entity_${entity.id}_${month}_statement.pdf`;
      a.click();
    ...
    <div className="space-y-4">
      ...
      >
        <Toolbar>
          <div>
            <Label>Entity</Label>
            {/* ✅ Searchable dropdown using names */}
...
function OwnerRentalTab() {
  ...
  const generateRowStatement = async (entityId, propertyName) => {
    await generateStatement(entityId, propertyName);
  };

  return (
    <div className="s health at a glance"
        right={
