...
// Add N months to YYYY-MM-DD and return YYYY-MM-DD (clamped end-of-month)
const addMonthsISO = (isoDate, months = 0) => {
  if (!isoDate) return '';
...
export default function PropertiesPage() {
  ...
  // prevent number fields from changing with wheel / arrows globally
  useEffect(() => {
    const wheelGuard = () => {
  ...
  // Track whether the lease end date has been edited manually
  const [leaseEndManuallyEdited, setLeaseEndManuallyEdited] = useState(false);
  ...
  const isOptionEqual = (a, b) => !!a && !!b && String(getContactId(a)) === String(getContactId(b));
  ...
  /* ----------------------- Lease end auto-populate ----------------------- */
  // Auto-populate Lease End Date when Lease Start Date changes (Rental purpose only)
  // unless the user manually edited the end date.
  useEffect(() => {
    if (form.purpose !== 'rental') return;
    if (!form.lease_start_date) return;
    if (leaseEndManuallyEdited) return;

    const startISO = toISODate(form.lease_start_date);
    const autoEnd = addMonthsISO(startISO, 11); // +11 months
    setForm(prev => ({ ...prev, lease_end_date: autoEnd }));
  }, [form.purpose, form.lease_start_date, leaseEndManuallyEdited]);
  ...
  const handleAddOrUpdateProperty = async () => {
    ...
    // Separate key_dates – they are handled separately
    const { key_dates, ...rest } = form;

    // Keys that should be sent as null when cleared
    ...
    const nullableFkKeys = new Set([
      'tenant',
      'project_manager',
      // landlord is required by validation, so we don't treat it as nullable here
    ]);

    const payload = {};
    Object.entries(rest).forEach(([key, val]) => {
      // undefined means "don't send"
      if (val === undefined) return;

      if (val === '' || val === null) {
        if (numericKeys.has(key) || dateKeys.has(key) || nullableFkKeys.has(key)) {
          payload[key] = null; // clear these fields on update
        } else {
          payload[key] = val;  // allow empty string for text fields (remarks, etc.)
        }
      } else {
        payload[key] = val;
      }
    ...
    try {
      ...
      if (isEditMode && editId) {
        // UPDATE with JSON
        response = await API.patch(`${EP.properties}${editId}/`, payload);
      } else {
        // CREATE with JSON
        response = await API.post(EP.properties, payload);
      }

      const propertyId = response.data.id;

      // ---- documents (multipart, because of files) ----
      if (documents.length > 0) {
        for (let doc of documents) {
          if (!doc.file) continue; // skip existing docs, they are already stored
  ...
  const resetForm = () => {
    ...
    setLeaseEndManuallyEdited(false);
  };
  ...
  const openEditDialog = (prop) => {
    ...
    // Treat existing end date as manual so we don't override it on edit
    setLeaseEndManuallyEdited(!!prop.lease_end_date);
  ...
  const updateKeyDateField = (idx, field, newValue) => {
    setForm(prev => {
      const next = [...(prev.key_dates || [])];
      next[idx] = { ...next[idx], [field]: field === 'due_date' ? toISODate(newValue) : newValue };
  ...
  // UPDATED: Show monthly rent for rentals, otherwise neutral/sale price
  // UPDATED: Show monthly rent for rentals, otherwise neutral/sale price
  const priceCell = (prop) => {
    if (prop.purpose === 'rental') {
      return prop.monthly_rent ?? '-';
    }
    if (prop.expected_price) {
      return prop.expected_price;
    }
      ...
      case 1:
        return (
          <Box p={2}>
            ...
            {form.purpose === 'rental' && (
              <>
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    const val = toISODate(e.target.value);
                    // allow auto-calc unless user edits end date after this
                    setLeaseEndManuallyEdited(false);
                    setForm({ ...form, lease_start_date: val });
                  }}
                  disabled={!canEdit}
                />
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    setLeaseEndManuallyEdited(true); // user overrides auto-fill
                    setForm({ ...form, lease_end_date: toISODate(e.target.value) });
                  }}
                  disabled={!canEdit}
                />
                ...
                <TextField
                  ...
                  onChange={(e) => setForm({ ...form, next_inspection_date: toISODate(e.target.value) })}
                  disabled={!canEdit}
                />
              </>
            )}
            ...
            {/* Neutral expected price + IGEN Service Charge */}
                                ...
                                <Table size="small">
                                  <TableBody>
                                    <TableRow>
                                      ...
                                      <TableCell>{prop.monthly_rent ? `₹${prop.monthly_rent}` : '-'}</TableCell>
                                      <TableCell sx={{ fontWeight: 'bold', width: '25%' }}>Expected Sale Price:</TableCell>
                                      <TableCell>{prop.expected_sale_price ? `₹${prop.expected_sale_price}` : '-'}</TableCell>
                                    </TableRow>
                                    <TableRow>
                                      <TableCell sx={{ fontWeight: 'bold' }}>Expected Price (Neutral):</TableCell>
                                      <TableCell>{prop.expected_price ? `₹${prop.expected_price}` : '-'}</TableCell>
                                      <TableCell sx={{ fontWeight: 'bold' }}>Service Charge:</TableCell>
                                      <TableCell>{prop.igen_service_charge ? `₹${prop.igen_service_charge}` : '-'}</TableCell>
                                    </TableRow>
                                    <TableRow>
