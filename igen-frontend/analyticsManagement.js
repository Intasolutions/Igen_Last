...
async function openEntityStatementDOCX(entityId, month) {
  if (!entityId) {
    alert("No entity linked to this row.");
    return;
  }
...
// --------- Owner / Property-based statement helpers (for Owner Dashboard rows) ---------
async function openOwnerPropertyStatementPDF(propertyId, month) {
  if (!propertyId) {
  ...
  try {
    const res = await API.get("analytics/owner-rental/property-statement/pdf/", {
      params: { property_id: propertyId, month },
      responseType: "blob",
      validateStatus: (s) => s >= 200 && s < 500,
    });
    ...
    const cd = res.headers?.["content-disposition"] || "";
    const m = cd.match(/filename="?([^"]+)"?/i);
    const filename = m?.[1] || `property_${propertyId}_${month}_statement.pdf`;
...
async function openOwnerPropertyStatementDOCX(propertyId, month) {
  if (!propertyId) {
    alert("Missing property id.");
    return;
  }
  try {
    ...
    const cd = res.headers?.["content-disposition"] || "";
    const m = cd.match(/filename="?([^"]+)"?/i);
    const filename = m?.[1] || `property_${propertyId}_${month}_statement.docx`;
...
function FinancialDashboard() {
  ...
  exportXlsx = async () => {
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      // use simple concatenation (no template literal) to avoid any escaping issues
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="flex items-center gap-2 text-sm col-span-2 mt-1">
                <input type="checkbox" checked={dateOn} onChange={() => setDateOn((v) => !v)} />
                date
                {dateOn && (
                  <select
...
function OwnerRentalTab() {
  ...
  const toggleFlag = async (id, fields, value) => {
    ...
    if (success && appliedKey) {
      setRows((prev) => prev.map((r) => (r.id === id ? { ...r, [appliedKey]: value } : r)));
    } else {
  ...
  const generateRowStatement = async (propertyId, format) => {
    ...
    try {
      const month = genCurrentMonth();
      if (format === "word") {
        await openOwnerPropertyStatementDOCX(propertyId, month);
      } else {
        await openOwnerPropertyStatementPDF(propertyId, month);
      }
    } catch (e) {
      console.error(e);
      alert("Failed to generate statement.");
